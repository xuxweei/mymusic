...
//        communityUp.setLike_status(Integer.parseInt(like_status));
        ...
        if (checked==null){
            ...
            if (checked.getLikeStatus() == 0){
                ...
                communityUp.setLikeStatus(0);
            }
            flag = communityUpService.update(communityUp);
        }
        if (flag) {
            if (communityUp.getLikeStatus() == 0 ){
                communityUp.setLikeStatus(-1);
            }
     ...
     * 点赞
     */
    @GetMapping(value = "/getLikedByCommunityId")
    public Object getLikedByCommunityId(HttpServletRequest request) {
        String consumer_id = request.getParameter("consumer_id").trim();
