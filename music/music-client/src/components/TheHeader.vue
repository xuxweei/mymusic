      ...
      <li class="noHover">
        <div class="header_search">
          ...
          <div class="search_btn" @click="goSearch()">
            <svg t="1608625769713" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg"
              p-id="4490" width="200" height="200">
              <path
              ...
              <path
                ...
                fill="#707070" p-id="4492"></path>
            </svg>
          </div>
        </div>
      </li>
      <li v-show="!loginIn" v-for="item in loginMsg" :key="item.path" @click="goPage(item.path,item.name)"
        :class="{active:item.name == activeName}">
        {{item.name}}
      </li>
    </ul>
