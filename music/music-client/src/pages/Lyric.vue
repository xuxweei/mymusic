<template>
    <div class="song_lyric">
      ...
      <!-- 有歌词 -->
      <ul class="has_lyric" v-if="lyrics.length" key="index">
          ...
          for(let i=0;i<this.lyrics.length;i++){
            ...
            if(this.currentTime>=this.lyrics[i][0]){
              ...
              for(let j=0;j<this.lyrics.length;j++){
                document.querySelectorAll('.has_lyric li')[j].style.color = '#000';
                document.querySelectorAll('.has_lyric li')[j].style.fontSize='14px';
              }
              if (i>=0) {
                document.querySelectorAll('.has_lyric li')[i].style.color = '#4AA5F0';
                document.querySelectorAll('.has_lyric li')[i].style.fontSize = '20px';   
              }
            }
          }
        }
